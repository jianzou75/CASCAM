...
#' @slot cell_aligned_data A data.frame of aligned cell line gene expression with rows for genes and columns for samples.
...
#' @slot cell_norm_data A data frame of normalized \emph{cell_aligned_data} with rows for samples and columns for genes.
#' @slot tumor_sda_project A one column matrix showing the SDA projected vector of tumor gene expression.
#' @slot cell_sda_project A one column matrix showing the SDA projected vector of tumor gene expression.
#' @slot sda_ds A data frame for SDA deviance score with the rows for cells and columns for tumor subtypes.
#' @slot sda_lds A data frame for SDA log deviance score with the rows for cells and columns for tumor subtypes.
#' @slot sda_predict_prob A two column matrix showing the SDA assignment probabilities for cells.
#' @slot sda_ds_pval A two column matrix showing the p-values of SDA deviance scores for cells.
...
#' @slot selected_cells The vector of genome-wide selected cell lines representing the interested tumor subtype by \emph{sda_predict_prob} and \emph{sda_ds_pval}.
...
#' @slot pathview_figure A pathview figure for one specific pathway and one specific cell line.
#'
#' @return
#' @export
#'
...
CASCAM <- methods::setClass(
  ...
  slots = c(
    ...
    cell_aligned_data = "data.frame",
    sda_model = "ANY",
    tumor_norm_data = "data.frame",
    cell_norm_data = "data.frame",
    tumor_sda_project = "matrix",
    cell_sda_project = "matrix",
    sda_ds = "data.frame",
    sda_lds = "data.frame",
